...
## 12. الـ Ideas Pipeline (v2.0 — جديد، مُحدّث في Session 7)

### 12.1 استلام أفكار فارس
...
### 12.2 ⚡ قاعدة فارس الإبداعية (Session 7 — صريحة وملزمة)

**القاعدة:** فارس قال صراحة:
> "اضافه السرقه الشرعيه القويه دي كفكره مني، فلو عندك حاجات زي كده ابداعيه باي شكل اعملها."

**معنى ذلك:**

- ✅ **أي مساهمة إبداعية أصيلة من فارس — حتى لو "بس share لورقة"** — تُسجَّل كـ `Idea-NNN` كاملة في الـ Ideas Bank.
- ✅ **أي ابتكار، اقتراح، ربط، فكرة جانبية، insight، أو ملاحظة حادة من فارس** تأخذ نفس المعاملة: ID رسمي + ملف تفصيلي + ربط شامل + citation tag.
- ✅ **النية الإبداعية = الفكرة.** فارس قد يرسل link، اسم theory، سؤال، شك، اقتراح، فكرة جانبية، إعادة framing... كلهم أفكار أصيلة منه.
- ✅ **أي شيء أبتكره أنا (الـ agent) بناءً على فكرة فارس يُنسب لها صراحة** بـ `[Idea-NNN]` في الورقة.

#### أمثلة على ما يُعتبر فكرة إبداعية من فارس

| نوع الفكرة | مثال | الحالة |
|---|---|---|
| Share لورقة بحثية | "شوف دي arxiv.org/abs/..." | ✅ Idea-NNN كاملة (مثل Idea-001 LEAP) |
| اقتراح نظري | "ابني نظرية عن X" | ✅ Idea-NNN |
| سؤال فلسفي | "ماذا يعني Y في سياقنا؟" | ✅ Idea-NNN |
| إعادة framing | "احنا فاكرين Z غلط، الصح كذا" | ✅ Idea-NNN |
| ملاحظة حادة | "لاحظ إن النتيجة دي تشبه..." | ✅ Idea-NNN |
| ربط بمشروع آخر | "ده زي ما عمل DeepMind في..." | ✅ Idea-NNN |
| تغيير mode أو direction | "خلينا نركز على Y بدل X" | ✅ Idea-NNN + protocol update |
| قاعدة جديدة للعمل | "من دلوقتي اعمل كذا" | ✅ Idea-NNN + protocol update (مثل هذه القاعدة نفسها = Idea-002) |

#### القرار الإلزامي

**لا أعتبر أي إسهام من فارس "صغير" أو "side note".** إذا فارس بذل جهد ذهني لـ:
- البحث عن مرجع
- صياغة سؤال
- اقتراح ربط
- توجيه مسار

→ هذا **إسهام إبداعي يستحق الـ trace الكامل في الـ Ideas Bank**.

#### Attribution Discipline

كل فكرة:
- لها ID رقمي تتابعي (لا يُعاد استخدامه).
- لها ملف تفصيلي بـ slug واضح.
- تُربط بكل theory/philosophy/section/figure/table يستخدمها.
- تُسجَّل في `PAPER/ideas/ATTRIBUTION_MAP.md` (جديد) لتتبع تأثيرها على الورقة.

#### السبب الفلسفي (مهم للنشر النهائي)

الورقة منتج بحثي مشترك بيني وبين فارس. التمييز بين:
- **أفكار فارس** (المالك الأصلي للمشروع، صاحب الرؤية).
- **توسيعاتي الفنية** (الـ agent ينفّذ ويوسّع).

هو شرط للأمانة العلمية. كل إسهام لفارس يجب أن يكون **traceable** في الورقة النهائية.
