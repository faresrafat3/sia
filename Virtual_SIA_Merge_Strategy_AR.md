# Virtual-SIA - استراتيجية الدمج النهائية
...
- **المخرجات:** virtual_sia/ package قابل للتثبيت، pytest working، CI-ready
...
### Runtime Core (virtual_sia/runtime/)
...
### Evaluation Framework (virtual_sia/eval/)
...
### Production API (virtual_sia/api/)
...
- [ ] التاكد من عدم وجود import errors: `python -c "import virtual_sia"`
...
- [ ] التأكد من بنية الـ import: `python -c "from virtual_sia.api.app import create_app; from virtual_sia.eval.runners.run_full_integration import run_full_integration"`
- [ ] تشغيل API في mock mode: `python -c "from virtual_sia.api.app import create_app; s = create_app(); print('Server ready on', s.server_address)"`
- [ ] تأكد من تشغيل eval runners: `python -c "from virtual_sia.eval.runners.run_condition import run_condition; r = run_condition('baseline_0', ['test task']); print('Success rate:', r.aggregate_metrics['success_rate'])"`
