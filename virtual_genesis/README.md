...
python -m virtual_genesis.eval.runners.run_adversarial_llm_eval   # أول دليل غير متشبّع: A=0% → B/C=50%
```

### شرائح/أدوات إضافية (تشخيصية)
```bash
python -m virtual_genesis.eval.runners.run_local_eval_v4              # شريحة تشخيصية
python -m virtual_genesis.eval.runners.run_selectivity_ablation       # معايرة المفاهيم
